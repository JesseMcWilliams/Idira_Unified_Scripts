...
$ModuleMeta = @{
    ...
    Category         = 'Accounts'
    Action           = 'Unlock'
    Description      = 'Unlock an account that has been locked due to password failures.'
...
function Invoke-AccountsUnlock {
    [CmdletBinding()]
    param(
    ...
    Write-CyberArkLog -Level 'INFO'  -Message "Starting unlock account for account ID: $accountId"
    Write-CyberArkLog -Level 'DEBUG' -Message "POST /API/Accounts/$accountId/Unlock"

    if ($WhatIf.IsPresent) {
        Write-CyberArkLog -Level 'INFO' -Message "WhatIf: POST /API/Accounts/$accountId/Unlock would be performed."
        $result.Successes++
    ...
    $response = Invoke-CyberArkAPI `
        -Token    $Token `
        -Method   'POST' `
        -Endpoint "/API/Accounts/$encodedId/Unlock" `
        -WhatIf:  $WhatIf.IsPresent

    if (-not $response.IsSuccess) {
        $msg = "Unlock Account failed (HTTP $($response.StatusCode)): $($response.ErrorMessage)"
    ...
    Write-CyberArkLog -Level 'INFO' -Message "Unlock Account complete for account ID: $accountId."
