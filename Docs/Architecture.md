...
| Linux SSH test tries plink before the PS7 SSH transport | `Test-LinuxSshAuth` now checks for plink first (via the new `Find-PlinkExecutable`: PATH, then the project root, then `%ProgramFiles(x86)%\PuTTY`, then `%ProgramFiles%\PuTTY`) and only falls back to `pwsh`'s SSH transport if plink isn't found | Confirmed live, 2026-09-04: even with the host-key fix above, a real password-auth attempt via the PS7 path still timed out - native OpenSSH needs a console for its password prompt, and this path has none. Plink's own `-pw` flag submits the password directly, sidestepping that entirely, and was confirmed live to fail in under a second (not hang) with a clear message when its own separate host-key cache doesn't yet have the target host - unlike OpenSSH, PuTTY has no CLI flag to auto-accept an unknown key, so this module doesn't try to work around that; the message it produces already tells the user what happened. |
