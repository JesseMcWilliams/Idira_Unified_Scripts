...
| Custom / TestConnectivity | Yes (orchestration mocked; DNS/TCP helpers and the `ConvertTo-Win32QuotedArgument` quoting logic exercised for real - no CyberArk connection needed for those) | `Unit\Invoke-CustomTestConnectivity.Tests.ps1` | Partially — the `ArgumentList`-crash fix (F23) and unknown-host-key hang fix (F25) were both confirmed live against a real test host (172.21.20.14); the actual SMB (`New-SmbMapping`) auth attempt and a *successful* Linux password auth are still not exercised by any automated or live test. See the module's own code comments for the PS7-SSH-transport password-auth limitation (native OpenSSH does not support non-interactive password auth reliably without a console - confirmed live that this can still cause a timeout even with a known host key; plink.exe is the documented, more reliable fallback for that specific case). Also confirm the `Safe`/`Username`/`PasswordSource` output columns (F22) against a real vault-backed lookup, and the Address-in-filename change (F26) for a real single interactive run |
...
- [ ] ExportAll (per user request, this session, now also runs Applications/ListAuthMethods -
      ...
      **F23 this session — was a `[FATAL] PropertyNotFoundException` crash on every Linux SSH
      ...
      F25 this session — confirm a Linux auth attempt with a *correct* password against a real
      host succeeds rather than timing out (only the unknown-host-key hang and an unrelated,
      still-unresolved password-prompt-without-a-console limitation were confirmed live; a
      correct-password success case was not); F26 this session — confirm the auto-saved CSV
      filename includes the tested Address for a real single interactive run)
