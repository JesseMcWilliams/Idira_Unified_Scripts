...
- **SelfHosted-only (3 modules):** `Platforms/Invoke-PlatformsRename.ps1` (confirmed via psPAS's
  ...
  (confirmed the same way, PVWA 14.6+ - mutating tenant-wide policy on an unconfirmed ISPSS
  endpoint was judged too risky to attempt speculatively), and `Reports/Invoke-ReportsList.ps1`.
  These are hidden from the menu entirely for an ISPSS profile.
  - **`Policies/Invoke-PoliciesGetMasterPolicy.ps1` is dual-use, unlike `SetMasterPolicy`** — per
    user request (to include Master Policy in `Custom/ExportAll`), even though psPAS's own
    `Get-PASMasterPolicy.ps1` explicitly asserts `-SelfHosted`. No ISPSS/Privilege Cloud
    equivalent endpoint is documented in any local CyberArk reference material, but nothing there
    confirms Privilege Cloud actually lacks it either - it's simply undocumented. Rather than
    guessing either way, it's declared dual-use and relies on this module's existing non-fatal
    failure handling (a 404 or other error is a `Failure`, not a crash, and not `IsFatal` unless
    401/0) - the same response-driven-fallback approach already used elsewhere in this project for
    endpoints with unconfirmed cross-system availability. **Read-only retrieval on ISPSS is
    live-unconfirmed** (no Privilege Cloud tenant available to test against); read on Self-Hosted
    remains fully live-confirmed as before. See Finding F40.
...
| Policies / GetMasterPolicy | Yes (incl. PGMP08/PGMP09 for the new dual-use/graceful-404 behavior) | `Unit\Invoke-PoliciesGetMasterPolicy.Tests.ps1` | Self-Hosted **confirmed (2026-09-04)** — live retrieval against the real tenant, including through the new `Custom/ExportAll` `IncludeInExportAll` path. ISPSS/Privilege Cloud is unverified: psPAS's own `Get-PASMasterPolicy.ps1` asserts Self-Hosted-only and no local reference material documents a Privilege Cloud equivalent, but nothing confirms it's absent either - per user direction, declared dual-use anyway and any failure there is handled as a normal non-fatal `Failure` rather than a crash. See Finding F40 |
...
- `POST /API/UserGroups/{id}/Members` (`Invoke-GroupsAddMember`) returns an unconditional, empty-body HTTP 400 on this tenant regardless of payload shape, field casing, or member type - tried the module's own shape, psPAS's exact shape, and several variants, all identical.
- `POST /API/Accounts/{id}/LinkAccount` and its bulk sibling `POST /API/Accounts/Link/Bulk` (`Invoke-AccountsLinkAccount`/`Invoke-AccountsUnlinkAccount`) both return HTTP 404 on this tenant even against a freshly confirmed-to-exist account, on both endpoint casings.
...
| K08 | `Invoke-GroupsAddMember.ps1` | `POST /API/UserGroups/{id}/Members` returned an unconditional, empty-body HTTP 400 on the 2026-09-04 live test tenant regardless of payload shape, field casing, or member type - reproduced with the module's own request shape, psPAS's `Add-PASGroupMember.ps1`'s exact shape, and several variants (int vs. string `memberId`, with/without `memberType`), all identical. Not fixed - no aPePAS-side request shape was found that succeeds; this may be a PVWA-version restriction, a disabled feature, or a WAF/gateway rule this project has no visibility into. | Re-test against a different Self-Hosted tenant/PVWA version; if it also fails there, escalate to CyberArk support with the raw 400 response for the actual cause, since no in-repo code change was found to resolve it. |
| K09 | `Invoke-AccountsLinkAccount.ps1` / `Invoke-AccountsUnlinkAccount.ps1` | `POST /API/Accounts/{id}/LinkAccount` and the bulk equivalent `POST /API/Accounts/Link/Bulk` both returned HTTP 404 on the 2026-09-04 live test tenant against a freshly confirmed-to-exist account (verified via a direct `GET` on that same account ID immediately before). Not fixed - both endpoint shapes documented by psPAS (`Set-PASLinkedAccount.ps1`) were tried and both 404d identically. | Re-test against a different Self-Hosted tenant/PVWA version to determine whether this is version-specific; check with CyberArk support whether the Link Account feature requires a license/config flag this tenant lacks. |
...
### Policies (2 actions — PVWA 14.6+; SetMasterPolicy is Self-Hosted only, GetMasterPolicy is dual-use)
- [x] GetMasterPolicy (F40 this session — **Self-Hosted confirmed live**, including via the new
      `Custom/ExportAll` `IncludeInExportAll` path; ISPSS/Privilege Cloud attempt is unconfirmed,
      no Privilege Cloud tenant available to test against - a failure there is expected to be
      handled gracefully as a non-fatal result, not confirmed live) ·
