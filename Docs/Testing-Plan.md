...
- ~~`POST /API/Accounts/{id}/LinkAccount` and its bulk sibling returned HTTP 404~~ — **resolved, see Finding F41 below.** The user has since confirmed live that both the single and bulk endpoints now work correctly against the real tenant; this was an environment-side condition at the time, not a code bug, and required no aPePAS change.
