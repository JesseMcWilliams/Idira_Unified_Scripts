...
| K10 | `Invoke-SafesDelete.ps1` | `DELETE /API/Safes/{safeName}` returned HTTP 409 on the 2026-09-04 live test tenant for three safes whose own `GET` response showed `"accounts": []` (confirmed empty) shortly after accounts had been added to and then deleted from them. Retried after a 15-second wait with the same result - not clearly a short transient delay. Not fixed - this is a plain, correct `DELETE` call with no equivalent "force" parameter in psPAS's `Remove-PASSafe.ps1` either, so no aPePAS-side change was identified. Three safes on that tenant (`ZZ-ClaudeTest-Safe1`, `ZZ-ClaudeTest-DiagSafeLink`, `ZZ-ClaudeTest-DiagSafeLink2`) were left in this stuck state and need manual deletion via the PVWA UI or a later retry once whatever internal state clears. | Retry deletion after a longer wait (hours, not seconds) to determine if this is a delayed internal cleanup; if it persists indefinitely, escalate to CyberArk support - likely an account-history/audit retention lock this project has no API-visible way to inspect or override. |
