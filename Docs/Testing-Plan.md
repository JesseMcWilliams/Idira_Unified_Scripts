...
- [ ] ExportAll (per user request, this session, now also runs Applications/ListAuthMethods -
      ...
      **F23 this session — was a `[FATAL] PropertyNotFoundException` crash on every Linux SSH
      attempt, reported directly by the user; needs re-confirmation that a real plink/PS7 SSH
      auth attempt now succeeds instead of crashing**; F24 this session — confirm the Server Type
      prompt shows and accepts only `1`/`2`)
