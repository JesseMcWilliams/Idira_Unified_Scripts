...
<#
.SYNOPSIS
    Unit tests for Invoke-AccountsUnlock.
#>

BeforeAll {
    $script:ModulePath  = Join-Path $PSScriptRoot '..\..\APIModules\Accounts\Invoke-AccountsUnlock.ps1'
    ...
    . $script:ModulePath
    Initialize-CyberArkLog -Destination 'Console' -ProfileName 'AccountsUnlockTests' -MinLevel 'ERROR'
}

Describe 'Invoke-AccountsUnlock' {
    ...
    Context 'Missing AccountID' {
        It 'returns failure when AccountID is not provided' {
            ...
            $result = Invoke-AccountsUnlock -Token $token -InputData @{}
    ...
    Context 'API call failure' {
        It 'records error on non-success response' {
            ...
            $result = Invoke-AccountsUnlock -Token $token -InputData @{ AccountID = 'acc123' }
    ...
    Context 'WhatIf mode' {
        It 'does not call the API when WhatIf is set' {
            ...
            { Invoke-AccountsUnlock -Token $token -InputData @{ AccountID = 'acc123' } -WhatIf } | Should -Not -Throw
        }
    }
