...
<#
.SYNOPSIS
    Unit tests for Invoke-AccountsChangeImmediate.
#>

BeforeAll {
    $script:ModulePath  = Join-Path $PSScriptRoot '..\..\APIModules\Accounts\Invoke-AccountsChangeImmediate.ps1'
    ...
    . $script:ModulePath
    Initialize-CyberArkLog -Destination 'Console' -ProfileName 'AccountsChangeImmediateTests' -MinLevel 'ERROR'
}

Describe 'Invoke-AccountsChangeImmediate' {
    ...
    Context 'Missing AccountID' {
        It 'returns failure when AccountID is not provided' {
            ...
            $result = Invoke-AccountsChangeImmediate -Token $token -InputData @{}
    ...
    Context 'API call failure' {
        It 'records error on non-success response' {
            ...
            $result = Invoke-AccountsChangeImmediate -Token $token -InputData @{ AccountID = 'acc123' }
    ...
    Context 'WhatIf mode' {
        It 'does not call the API when WhatIf is set' {
            ...
            { Invoke-AccountsChangeImmediate -Token $token -InputData @{ AccountID = 'acc123' } -WhatIf } | Should -Not -Throw
        }
    }
