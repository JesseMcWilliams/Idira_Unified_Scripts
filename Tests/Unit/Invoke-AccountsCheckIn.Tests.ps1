...
<#
.SYNOPSIS
    Unit tests for Invoke-AccountsCheckIn.
#>

BeforeAll {
    $script:ModulePath  = Join-Path $PSScriptRoot '..\..\APIModules\Accounts\Invoke-AccountsCheckIn.ps1'
    ...
    . $script:ModulePath
    Initialize-CyberArkLog -Destination 'Console' -ProfileName 'AccountsCheckInTests' -MinLevel 'ERROR'
}

Describe 'Invoke-AccountsCheckIn' {
    ...
    Context 'Missing AccountID' {
        It 'returns failure when AccountID is not provided' {
            ...
            $result = Invoke-AccountsCheckIn -Token $token -InputData @{}
    ...
    Context 'API call failure' {
        It 'records error on non-success response' {
            ...
            $result = Invoke-AccountsCheckIn -Token $token -InputData @{ AccountID = 'acc123' }
    ...
    Context 'WhatIf mode' {
        It 'does not call the API when WhatIf is set' {
            ...
            { Invoke-AccountsCheckIn -Token $token -InputData @{ AccountID = 'acc123' } -WhatIf } | Should -Not -Throw
        }
    }
